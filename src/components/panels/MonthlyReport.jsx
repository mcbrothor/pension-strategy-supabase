...
export default function MonthlyReport({ portfolio, vix, vixSource, vixUpdatedAt, vixError }) {
  ...
  const generateInsight = () => {
    ...
    if (performance.unrealizedReturn != null) {
      text += ` 원가 기준 평가손익률은 ${formatPercentValue(performance.unrealizedReturn)}입니다.`;
    }
    return text;
  };
  ...
  function print() {
    ...
    <h3>1. 개인 성과 요약</h3>
    <table><tbody>
      <tr><th>원가 합계</th><td>${performance.costTotal > 0 ? `${fmt(performance.costTotal)}원` : "원가 입력 필요"}</td><th>평가손익</th><td>${performance.unrealizedPnl != null ? `${performance.unrealizedPnl >= 0 ? "+" : "-"}${fmt(Math.abs(performance.unrealizedPnl))}원 (${formatPercentValue(performance.unrealizedReturn)})` : "원가 입력 필요"}</td></tr>
      <tr><th>스냅샷 기준 성과</th><td>${performance.hasHistory ? `${performance.firstDate} 대비 ${formatPercentValue(performance.periodReturn)}` : "스냅샷 부족"}</td><th>연환산 단순 성과</th><td>${formatPercentValue(performance.annualizedReturn)}</td></tr>
      <tr><th>전략 기대수익률 환산</th><td>${performance.benchmarkPeriodReturn != null ? `${performance.periodDays}일 기준 ${formatPercentValue(performance.benchmarkPeriodReturn)}` : "기간 산정 불가"}</td><th>기대수익률 대비</th><td>${formatPercentValue(performance.benchmarkGap)}</td></tr>
    </tbody></table>
    <div style="font-size:11px;color:#666;margin-top:-8px">※ 개인용 단순 성과입니다. 외부 입출금, 세금, 배당, 실제 체결가격은 별도로 반영되지 않습니다.</div>
      ...
      <Card>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: "1.5rem" }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 4 }}>성과 요약, 리스크 지표, 매니저 인사이트 포함</div>
          </div>
        ...
        <div style={{ background: "var(--bg-main)", borderRadius: "var(--radius-md)", padding: "1.5rem", fontSize: 12, lineHeight: 1.8 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10, marginBottom: "1.5rem" }}>
            <div style={{ background: "var(--bg-card)", borderRadius: 8, padding: "12px 14px" }}>
              <div style={{ fontSize: 10, color: "var(--text-dim)", marginBottom: 4 }}>원가 기준 평가손익률</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: performance.unrealizedReturn != null && performance.unrealizedReturn < 0 ? "#a32d2d" : "#3b6d11" }}>
                {formatPercentValue(performance.unrealizedReturn)}
              </div>
              <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 3 }}>
                {performance.unrealizedPnl != null ? `${performance.unrealizedPnl >= 0 ? "+" : "-"}${fmt(Math.abs(performance.unrealizedPnl))}원` : "원가 입력 필요"}
              </div>
            </div>
            <div style={{ background: "var(--bg-card)", borderRadius: 8, padding: "12px 14px" }}>
              <div style={{ fontSize: 10, color: "var(--text-dim)", marginBottom: 4 }}>스냅샷 기준 기간 성과</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: performance.periodReturn != null && performance.periodReturn < 0 ? "#a32d2d" : "#185fa5" }}>
                {formatPercentValue(performance.periodReturn)}
              </div>
              <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 3 }}>
                {performance.hasHistory ? `${performance.firstDate} 이후 ${performance.periodDays}일` : "스냅샷 부족"}
              </div>
            </div>
            <div style={{ background: "var(--bg-card)", borderRadius: 8, padding: "12px 14px" }}>
              <div style={{ fontSize: 10, color: "var(--text-dim)", marginBottom: 4 }}>전략 기대수익률 대비</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: performance.benchmarkGap != null && performance.benchmarkGap < 0 ? "#a32d2d" : "#3b6d11" }}>
                {formatPercentValue(performance.benchmarkGap)}
              </div>
              <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 3 }}>
                {performance.benchmarkPeriodReturn != null ? `기대 ${formatPercentValue(performance.benchmarkPeriodReturn)}` : "기간 산정 불가"}
              </div>
            </div>
          </div>
