...
export const STRATEGIES = [
  { id:"60_40", type:"fixed", level:"초급", name:"60/40 포트폴리오", ret:"약 8%", mdd:"약 30%", rebal:"연 1회", irpRisk:60,
    ...
    desc:"가장 기본적인 주식·채권 분산. 대표적인 성장 자산인 미국 주식과 변동성을 완충하는 채권에 배분합니다.",
    composition:[{cls:"미국주식",w:60,risk:true},{cls:"미국중기채권",w:40,risk:false}]},
  { id:"permanent", type:"fixed", level:"초급", name:"영구 포트폴리오", ret:"약 8%", mdd:"약 12.7%", rebal:"연 1회", irpRisk:50,
    ...
    desc:"해리 브라운의 전천후 전략. 주식·장기채·금·현금에 25%씩 균등 배분하여 어떤 경제 상황에서도 자산을 보호합니다.",
    composition:[{cls:"미국주식",w:25,risk:true},{cls:"미국장기채권",w:25,risk:false},{cls:"금",w:25,risk:true},{cls:"현금MMF",w:25,risk:false}]},
  { id:"permanent_global", type:"fixed", level:"초급", name:"영구 포트폴리오 글로벌", ret:"약 8%", mdd:"약 15%", rebal:"연 1회", irpRisk:62.5,
    annualRet:{cons:0.055,base:0.075,opt:0.090},
    desc:"영구 포트폴리오의 글로벌 분산 버전. 미국 외 신흥국·국내 주식을 추가해 지역 리스크를 낮춥니다.",
    composition:[{cls:"미국주식",w:12.5,risk:true},{cls:"신흥국주식",w:12.5,risk:true},{cls:"미국장기채권",w:12.5,risk:false},{cls:"글로벌국채",w:12.5,risk:false},{cls:"금",w:25,risk:true},{cls:"현금MMF",w:12.5,risk:false},{cls:"국내주식",w:12.5,risk:true}]},
  { id:"allseason", type:"fixed", level:"초급", name:"사계절 포트폴리오", ret:"8~10%", mdd:"약 13.1%", rebal:"연 1회", irpRisk:45,
    ...
    desc:"레이 달리오의 대표 전략. 경제 4계절(성장/하락, 인플레이션/디플레이션)을 모두 대비하는 자산 배분입니다.",
    composition:[{cls:"미국주식",w:30,risk:true},{cls:"미국중기채권",w:15,risk:false},{cls:"미국장기채권",w:40,risk:false},{cls:"금",w:7.5,risk:true},{cls:"원자재",w:7.5,risk:true}]},
  { id:"allweather", type:"fixed", level:"중급", name:"올웨더 2유형", ret:"8~10%", mdd:"20% 이하", rebal:"연 1회", irpRisk:50,
    annualRet:{cons:0.06,base:0.085,opt:0.105},
    desc:"글로벌 주식 3종 + 원자재·금 + 장기채·물가연동채·회사채·신흥국채권을 결합한 정교한 분산 전략입니다.",
    composition:[{cls:"미국주식",w:12,risk:true},{cls:"선진국주식",w:12,risk:true},{cls:"신흥국주식",w:12,risk:true},{cls:"원자재",w:7,risk:true},{cls:"금",w:7,risk:true},{cls:"미국장기채권",w:18,risk:false},{cls:"물가연동채",w:18,risk:false},{cls:"회사채",w:7,risk:false},{cls:"신흥국채권",w:7,risk:false}]},
  ...
  { id:"laa", type:"momentum", level:"고급", name:"LAA (게으른 자산배분)", ret:"약 10%", mdd:"약 13%", rebal:"월 1회", irpRisk:75,
    ...
    desc:"75%를 고정 보유하고 25%만 모멘텀 운용하는 하이브리드 전략. 200일 이동평균과 실업률로 시장 국면을 판단합니다.",
    calc:"200일 이동평균(200DMA) + 실업률 듀얼 조건:\n조건1: SPY 현재가 > 200일 이평균\n조건2: 미국 실업률 < 12개월 평균\n→ 둘 다 충족: 공격자산(미국주식) 25%\n→ 하나라도 미충족: 방어자산(현금MMF) 전환\n※ 데이터 조회 실패 시 12M 모멘텀으로 자동 폴백",
    composition:[
